...
mod tests {
    ...
    #[test]
    fn test_language_id() {
        assert_eq!(language_id(), "typescript");
    }
    ...
    fn test_completions_prefix_match() {
        let result = get_completions("in", 0, 2);
        let items: Vec<serde_json::Value> = serde_json::from_str(&result).unwrap();
        assert!(items.len() > 0);
        for item in &items {
            let label = item["label"].as_str().unwrap().to_lowercase();
            assert!(label.starts_with("in"), "label '{}' does not start with 'in'", label);
        }
    }
    ...
    fn test_hover_known_keyword() {
        let result = get_hover("interface Foo {}", 0, 1);
        assert_ne!(result, "null");
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
        assert!(parsed["contents"].as_str().unwrap().contains("Declares an interface."));
    }
